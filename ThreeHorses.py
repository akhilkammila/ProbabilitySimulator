...
"""
def generalized_horses(X, Y):
    samples = 1000000
    dif1 = norm.ppf(X) * np.sqrt(2)
    dif2 = norm.ppf(Y) * np.sqrt(2)
    a = np.random.normal(dif1, 1, size=samples)
    b = np.random.normal(0, 1, size=samples)
    c = np.random.normal(-dif2, 1, size=samples)
    print(np.count_nonzero(a >= b)/samples)
    print(np.count_nonzero(b >= c)/samples)
    print(np.count_nonzero(a >= c)/samples)

generalized_horses(0.6, 0.7)